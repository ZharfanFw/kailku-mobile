...
export default function PaymentScreen() {
  ...
  const [expandedSection, setExpandedSection] = useState<string | null>('virtual_account');
  ...
  const paymentMethods: PaymentCategory[] = [
    {
      ...
      title: 'Qris',
      data: [
        { id: 'qris_code', name: 'Qris', image: 'https://via.placeholder.com/50x50/000000/FFFFFF?text=QRIS' },
      ]
    },
    {
      id: 'virtual_account',
      ...
      data: [
        { id: 'bca', name: 'BCA', image: 'https://via.placeholder.com/50x50/005596/FFFFFF?text=BCA' },
        { id: 'bni', name: 'BNI', image: 'https://via.placeholder.com/50x50/F15A23/FFFFFF?text=BNI' },
        { id: 'mandiri', name: 'Mandiri', image: 'https://via.placeholder.com/50x50/003D79/FFFFFF?text=M' },
        { id: 'bsi', name: 'BSI', image: 'https://via.placeholder.com/50x50/00A39D/FFFFFF?text=BSI' },
      ]
    }
  ];
  ...
  const handlePay = () => {
    ...
    if (selectedMethod === 'Qris') {
      router.push('/Qris'); // Pindah ke file Qris.tsx
      return;
    }
    ...
    paymentMethods.forEach(category => {
        const foundItem = category.data.find(item => item.name === selectedMethod);
        if (foundItem) {
    ...
    <SafeAreaView style={styles.container}>
            ...
            <View key={category.id} style={styles.sectionContainer}>
              ...
              {isOpen && (
                <View style={styles.sectionBody}>
                  {category.data.map((method, methodIndex) => {
                     const isSelected = selectedMethod === method.name;
                     // Cek apakah ini item terakhir untuk styling border
                     const isLast = methodIndex === category.data.length - 1;

                     return (
                       <TouchableOpacity
                         key={method.id}
                         style={[styles.paymentItem, isLast && { borderBottomWidth: 0 }]}
      ...
      <View style={styles.footer}>
        <View style={styles.footerRow}>
          ...
          <Text style={styles.footerPrice}>{formatRupiah(totalPayment)}</Text>
        </View>
