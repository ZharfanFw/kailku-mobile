...
export default function BuysAndRentFishingScreen() {
  ...
  const products: Product[] = [
    { id: '1', name: 'Joran Baitcasting', price: 152000, discount: 25, image: 'https://via.placeholder.com/150/0000FF/808080?text=Joran' },
    { id: '2', name: 'PE Braided Line', price: 53000, discount: 64, image: 'https://via.placeholder.com/150/FF0000/FFFFFF?text=Line' },
    { id: '3', name: 'Spoon Pancing', price: 25000, discount: 23, image: 'https://via.placeholder.com/150/FFFF00/000000?text=Spoon' },
    { id: '4', name: 'Tas Pancing', price: 152000, discount: 73, image: 'https://via.placeholder.com/150/008000/FFFFFF?text=Tas' },
    { id: '5', name: 'Tanggok Jaring', price: 152000, discount: 62, image: 'https://via.placeholder.com/150/000000/FFFFFF?text=Jaring' },
    { id: '6', name: 'Ember Pancing', price: 152000, discount: 43, image: 'https://via.placeholder.com/150/800080/FFFFFF?text=Ember' },
  ];
  ...
  const removeFromCart = (productId: string, type: 'sewa' | 'beli') => {
    ...
    const index = cart.findIndex(item => item.id === productId && item.type === type);
  ...
  const getItemCount = (productId: string, type: 'sewa' | 'beli') => {
    return cart.filter(item => item.id === productId && item.type === type).length;
  };
  ...
  const renderCounterButton = (item: Product, type: 'sewa' | 'beli', count: number) => {
      ...
      {cart.length > 0 && (
        <View style={styles.footerCard}>
          <View style={styles.totalRow}>
            <View>
              <Text style={styles.totalLabel}>Total ({cart.length} Item)</Text>
              <Text style={styles.totalPrice}>{formatRupiah(totalCartPrice)}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  discountBadge: { position: 'absolute', top: 0, right: 0, backgroundColor: '#DC2626', paddingHorizontal: 8, paddingVertical: 4, borderTopRightRadius: 12, borderBottomLeftRadius: 8, zIndex: 1 },
  discountText: { color: '#FFF', fontSize: 10, fontWeight: 'bold' },
  productImage: { width: '100%', height: 100, marginBottom: 10, borderRadius: 8, backgroundColor: '#F5F5F5' },
  cardContent: { flex: 1 },
  productName: { fontSize: 12, fontWeight: '600', color: '#333', marginBottom: 4 },
