    ...
    fastify.post("/signup", async (request, reply) => {
        ...
        if (!first_name || !last_name || !email || !password) {
            return reply.status(400).send({
                message:
                    "Nama depan, nama belakang, email, dan password wajib diisi",
            });
        }
    ...
    fastify.post("/login", async (request, reply) => {
        ...
        try {
            ...
            if (rows.length === 0) {
                return reply
                    .status(401)
                    .send({ message: "Email atau Password salah" });
            }
    ...
    fastify.patch(
        ...
        async (request, reply) => {
            ...
            if (
                ...
                return reply
                    .status(400)
                    .send({ message: "Minimal kirim satu data" });
            }
            ...
            try {
                ...
                reply
                    .status(500)
                    .send({ message: "Terjadi error pada server" });
            }
        },
    );
    ...
    fastify.post("/forgot-password", async (request, reply) => {
        ...
        try {
            ...
            if (rows.length === 0) {
                ...
                return reply
                    .status(404)
                    .send({ message: "Email tidak terdaftar" });
            }
            ...
            return reply.send({
                message:
                    "Link reset password telah dikirim ke console backend!",
            });
        } catch (err) {
            if (connection) connection.release();
